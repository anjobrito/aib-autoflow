...
export function ServicesClient() {
  ...
  async function refresh() {
    ...
    try {
      ...
      const fallbackRows = filterServicesByBusinessProfile(demoServices, profile).map((service) => ({
        id: service.id,
        name: service.name,
        category: service.category,
        duration: service.duration,
  ...
  const rows = useMemo(() => {
    const visible = services.filter((service) => service.editable || filterServicesByBusinessProfile([service], profile).length > 0);
    if (!searchTerm.trim()) return visible;
    const term = normalize(searchTerm);
    return visible.filter((service) => normalize(`${service.name} ${service.category} ${service.duration} ${service.price} ${service.status}`).includes(term));
  }, [services, profile, searchTerm]);
    ...
    <div className="grid gap-6">
      <div className="flex flex-col gap-3 rounded-3xl bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Serviços reais agora são gravados por empresa no PostgreSQL. Modelos aparecem apenas quando a sessão/banco não estiver disponível.</p>
        </div>
